...
export default async function handler(req, res) {
  ...
  if (!userId || typeof userId !== 'string') {
    return res.status(400).json({ error: 'userId is required until authentication is connected' });
  }
  ...
  try {
    ...
    if (movie.status !== 'UPCOMING') {
      return res.status(409).json({ error: 'This movie is no longer in the pre-release Hype pool' });
    }

    const existing = await prisma.hypeVote.findUnique({
    ...
    const vote = await prisma.hypeVote.upsert({
      where: { userId_movieId: { userId, movieId: id } },
      update: { choice: choice.toUpperCase(), watchedAt: null, invalidatedAt: null, updatedAt: now },
      create: { userId, movieId: id, choice: choice.toUpperCase(), createdAt: now },
    });
    ...
    await prisma.hypeVoteAudit.create({
      data: {
        ...
        newChoice: vote.choice,
        action: existing ? 'changed' : 'created',
      },
    });

    const votes = await prisma.hypeVote.findMany({ where: { movieId: id } });

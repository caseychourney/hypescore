...
export default function MoviePage({movie:rawMovie}) {
  ...
  return <div className={styles.site}>
    ...
    <main className={styles.movieClean}>
      ...
      <section className={styles.infoBlock}><div className={styles.infoTabs}><button className={infoTab==='basic'?styles.infoActive:''} onClick={()=>setInfoTab('basic')}>Movie Info</button><button className={infoTab==='nerds'?styles.infoActive:''} onClick={()=>setInfoTab('nerds')}>For the Nerds 🤓</button><button className={infoTab==='cast'?styles.infoActive:''} onClick={()=>setInfoTab('cast')}>Cast & Crew</button></div>{infoTab==='basic'&&<div className={styles.infoGrid}><div><span>Release</span><strong>{movie.release}</strong></div><div><span>Runtime</span><strong>{movie.runtime}</strong></div><div><span>Rating</span><strong>{movie.rating}</strong></div><div><span>Genre</span><strong>{movie.genre}</strong></div><div><span>Budget</span><strong>{movie.budget}</strong></div><div><span>Worldwide Box Office</span><strong>{movie.boxOffice}</strong></div><div><span>Synopsis</span><strong>{movie.overview}</strong></div></div>}{infoTab==='nerds'&&<div className={styles.infoGrid}><div><span>Original Language</span><strong>{rawMovie.originalLanguage||'—'}</strong></div><div><span>Source</span><strong>{rawMovie.sourceProvider||'—'}</strong></div><div><span>TMDB ID</span><strong>{rawMovie.tmdbId||'—'}</strong></div><div><span>Trailer</span><strong>{movie.trailerUrl?'Available':'Not added'}</strong></div><div><span>Production Companies</span><strong>{Array.isArray(rawMovie.productionCompanies)?rawMovie.productionCompanies.map(x=>x.name||x).join(', '):'—'}</strong></div></div>}{infoTab==='cast'&&<div className={styles.infoGrid}>{movie.cast.slice(0,18).map((c,i)=><div key={c.id||i}><span>{c.person?.name||'Cast'}</span><strong>{c.characterName||'Cast'}</strong></div>)}{movie.crew.slice(0,12).map((c,i)=><div key={'crew-'+(c.id||i)}><span>{c.person?.name||'Crew'}</span><strong>{c.job||c.department}</strong></div>)}</div>}</section>
